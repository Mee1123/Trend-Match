...
##### 5.1.3 退会
理由：
+ システムを利用しないユーザー情報を保持し続けないようにするため.
...
説明：
 + ニックネーム,プロフィール画像,卒業年度,所属,職種,性別,連絡先,内定の有無,フリースペース,価値観を設定し,入力する. 
+ 下記にユーザーアカウントの登録をする際の制約を記す.
+ なお,アカウント情報登録の際には事前に個人情報の公開に同意する必要がある.

| 登録名 | データ形式 | 文字入力数 | その他制約 |
| ---- | ---- | ---- | ---- |
| ニックネーム|半角文字,全角文字|10以下|必須項目,デフォルトの場合はユーザー情報で登録した氏名が表示|
| プロフィール画像|画像||未設定の場合はデフォルト画像|
|卒業年度|半角数字|4文字|西暦で入力,必須項目|
| 所属|半角文字,全角文字|20以下||
| 職種|ドロップダウン|  |職種IDを用意 |
| 性別 | ラジオボタン||性別IDを用意
| 連絡先 | 半角文字 | 40以下 ||
...
+ ユーザーは主な価値観を3つまで入力する.(任意)
+ ユーザーの価値観を導き出すにあたっての自己分析方法を提示する.（画像つき）
    - 具体的な方法
    - モチベーショングラフの作成
    - 自分史の作成
    - 上記を踏まえどんな価値観をもって人生を送ってきたのかを考える
+ 個人のエニアグラムの結果はカスタマーに持たせる
+ 価値観は価値観データベースに保存されている価値観と,カスタマーを紐付ける.

| 登録名 | データ形式 | 文字入力数 | その他制約 |
| ---- | ---- | ---- | ---- |
|エニアグラム診断結果|半角数字||0~10で入力する<br>9項目分用意する|
| 価値観(3つまで)　|　半角文字,全角文字　|　50以下　|　打ち込んだ内容を含む,システムに登録されている価値観をドロップダウンリストで表示する<br>なお,登録されていない場合は5.1.11を行う　|
...
説明：
- 一覧画面で記載する情報はニックネーム,プロフィール画像,所属,職種,フリースペース内容の一部.
...
説明：
+ ユーザーは登録後にログインを行うことができる.利用者,管理者はメールアドレスとパスワードを入力し,認証を受けることでシステムに接続される.
...
+ 学生ユーザーの内定の有無が有の場合,アカウント名のあとに印をつける.
...
- 一覧画面で記載する情報はニックネーム,プロフィール画像,所属,職種,フリースペース内容の一部.
- 詳しく見たい人を選択すると,当該アカウントのアカウント情報が全てみられる.
