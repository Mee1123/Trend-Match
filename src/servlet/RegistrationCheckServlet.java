...
public class RegistrationCheckServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		/*
		 * if(form.getError().isEmpty()){ UserService service = new
		 * UserService(); try { service.RegistrationUser(request,form); } catch
		 * (Exception e) { // TODO: handle exception e.printStackTrace(); } if
		 * (form.getError().isEmpty()) {
		 * response.sendRedirect("/WEB-INF/checkUserRegistration"); } else {
		 * request.setAttribute("form", form); doGet(request, response); } }else
		 * { request.setAttribute("form", form); doGet(request, response); }
		 */
