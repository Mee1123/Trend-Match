...
public class accountRegistrationCheckServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (SessionHelper.sessionCheck(request, response)) {
			...
			accountRegistrationCheckForm form = new accountRegistrationCheckForm(jobOffer, nickname, graduate,
					department, occupation, sex, contact, freeSpace, value1, value2, value3);

			String newGraduate = graduate+"0331";


			// プロフィール画像
			/*
			 * try { Part part = request.getPart("file"); if(part != null){
			 * HttpSession session = request.getSession(); int userId = (int)
			 * session.getAttribute("userID");
			 * System.out.println("ファイルを受け付けます:userID="+userId); int name = 1;
			 * part.write(getServletContext().getRealPath("/WebContent/picture")
			 * + "/" + name); } }catch (ArithmeticException e) {
			...
			if (form.getError().isEmpty()) {
				...
				request.setAttribute("graduate", newGraduate);
